...
class LeopartTransforms:
    ...
    def __init__(self,
        ...
        color_transform = [
            torchvision.transforms.RandomApply([self.color_jitter], p=0.8),
        ...
        # self.final_transform = torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        # self.final_transform = torchvision.transforms.Compose([normalize])
        ...
        for i, rrc_transform in enumerate(self.rrc_transforms):
            ...
            if i > 0:
                ...
                while True:
                    ...
                    min_intersection = int((sample.shape[1] * sample.shape[2]) * self.min_intersection)
            ...
            img = F.resized_crop(sample, y1, x1, h, w, rrc_transform.size, rrc_transform.interpolation, antialias=True)
...
def get_transform(
        res, is_label, image_set, randomaug=False, use_leopart_transform=False,
        size_crops=None, nmb_crops=None, min_scale_crops=None, max_scale_crops=None, res_resize=None,
    ):
    if res is not None and isinstance(res, int):
        res = (res, res)

    if use_leopart_transform:
        return LeopartTransforms(
            size_crops,
            nmb_crops,
            min_scale_crops,
            max_scale_crops
    ...
    if res_resize:
        if is_label:
            ...
            return T.Compose([
                T.Resize(res_resize, interpolation=Image.BILINEAR, antialias=True),
                T.CenterCrop(res),
                # T.ToTensor(),
                # normalize
            ])
        ...
        if is_label:
            ...
            return T.Compose([
                T.Resize(res, interpolation=Image.BILINEAR, antialias=True),
                # T.ToTensor(),
                # normalize
            ])
