...
from .utils import PredsmIoU, get_backbone_weights
from .fcn.fcn_head import FCNHead
from .models import ResnetDilated
from .models import vit_small, vit_base
from .linear_finetuning_transforms import Compose, Normalize, RandomHorizontalFlip, RandomResizedCrop, ToTensor
...
def linear_finetune(_config, _run):
    ...
    if dataset_name == "voc":
        ...
        if mask_type == "thing":
            ...
            num_classes = 15
        ignore_index = 255
        file_list = os.listdir(os.path.join(data_dir, "cocostuff", "images", "train2017"))
        file_list_val = os.listdir(os.path.join(data_dir, "cocostuff", "images", "val2017"))
    ...
    trainer = Trainer(
        ...
        accelerator="gpu" if _config["gpus"] > 0 else "cpu",
        strategy="ddp" if _config["gpus"] > 1 else None,
...
class LinearFinetune(pl.LightningModule):
    ...
    def __init__(self, patch_size: int, num_classes: int, lr: float, input_size: int, spatial_res: int, val_iters: int,
        ...
        self.accuracy_metric = Accuracy(task="multiclass", num_classes=num_classes, ignore_index=ignore_index)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int):
        if batch_idx < self.val_iters:
            with torch.no_grad():
                ...
                self.accuracy_metric(mask_preds[valid].view(-1), gt[valid].view(-1).long())
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        print(f"mIoU: {miou}, Accuracy: {acc}")
