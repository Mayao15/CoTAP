...
class COCOSegmentation(VisionDataset):
    ...
    def __init__(
        ...
        if not os.path.isdir(seg_dir) or not os.path.isdir(image_dir):
            print(f"Seg dir: {seg_dir}, Exists: {os.path.isdir(seg_dir)}")
            print(f"Image dir: {image_dir}, Exists: {os.path.isdir(image_dir)}")
            print(f"Root contents ({root}): {os.listdir(root)}")
            if os.path.isdir(os.path.join(root, "annotations")):
                print(f"Annotations contents: {os.listdir(os.path.join(root, 'annotations'))}")
