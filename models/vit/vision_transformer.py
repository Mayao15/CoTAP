...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, embed_dim=768, output_dim=256, hidden_dim=2048,
                 ...
                 head_idx_patch=0, head_idx_cls=1, depth_in=3, depth_out=3, last_skip=1, feat_type_default='all', **kwargs):
        super().__init__()
        ...
        self.blocks_in = nn.ModuleList([nn.ModuleList([
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, \
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth_in)]) for _ in range(n_projection_head)])
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth_in], norm_layer=norm_layer)
            for i in range(depth - depth_in - depth_out)])
        self.blocks_out = nn.ModuleList([nn.ModuleList([
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, \
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth-depth_out], norm_layer=norm_layer)
            for i in range(depth_out)]) for _ in range(n_projection_head)])
        ...
        # self.blocks_split = nn.ModuleList([
        #     Block(
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        for i in range(n_projection_head):
            if head_idx_patch == head_idx_cls:
                ...
                feat_type = 'cls'
            projection_heads.append(Projector(embed_dim, hidden_dim[i], output_dim, \
        ...
        #     nn.Linear(output_dim, nmb_prototypes[i], bias=False)
        # for i in range(n_projection_head)])
    ...
    def forward(self, inputs, masks=None, last_self_attention=False, proj_head=None):
        results = {}
        _out = self.forward_backbone(inputs, masks, last_self_attention=last_self_attention, proj_head_idx=proj_head)
    ...
    def prepare_tokens(self, x, masks=None):
        ...
        if masks is not None:
            x = torch.where(masks.view(B, x.shape[1], 1), self.mask_token.to(x.dtype).unsqueeze(0), x)
    ...
    def forward_backbone(self, x, masks=None, last_self_attention=False, proj_head_idx=None):
        x = self.prepare_tokens(x, masks)
        x_chunk = x.chunk(len(self.projection_heads))
        for i in range(self.depth_in):
            x_chunk = [self.blocks_in[c][i](x_chunk[c]) for c in range(len(x_chunk))]
        x = torch.cat(x_chunk)
        ...
        if self.depth_out > 0:
            for blk in self.blocks:
                x = blk(x)
            # x_chunk = x.chunk(len(self.projection_heads))
            for i in range(self.depth_out - 1):
                x = self.blocks_out[proj_head_idx][i](x)
            out_list, attn_list = [], []

            out = self.blocks_out[proj_head_idx][-1](x, return_attention=last_self_attention)
            if last_self_attention:
                out, attn = out
                attn_list = attn[:, :, 0, 1:]
            out = self.norm(out)
            ...
            if self.last_skip == 0 or proj_head_idx == self.head_idx_cls:
                for blk in self.blocks[:-1]:
                    x = blk(x)
                out = self.blocks[-1](x, return_attention=last_self_attention)
            else:
                for blk in self.blocks[:-self.last_skip-1]:
                    x = blk(x)
                out = self.blocks[-self.last_skip-1](x, return_attention=last_self_attention)

            if last_self_attention:
                out, attn = out
                attn = attn[:, :, 0, 1:]
                # attn_list = attn.chunk(len(self.projection_heads))
                attn_list = attn
            out = self.norm(out)
            # out_list = out.chunk(len(self.projection_heads))
            out_list = out
    ...
    #     return self.blocks[-1](x, return_attention=True)[1]

    def get_last_feature(self, x):
        assert len(self.blocks_in[0]) == 0
        x = self.prepare_tokens(x)
        # x_chunk = [x, x]
        # for i in range(self.depth_in):
        #     x_chunk = [self.blocks_in[c][i](x_chunk[c]) for c in range(len(self.blocks_in))]
        # x = (x_chunk[0] + x_chunk[1]) / 2
        ...
        if self.depth_out > 0:
            ...
            for i in range(self.depth_out):
                x_chunk = [self.blocks_out[c][i](x_chunk[c]) for c in range(len(x_chunk))]
            out = x_chunk
        else:
            if self.last_skip == 0:
                for blk in self.blocks:
                ...
                for blk in self.blocks[:-self.last_skip]:
                    x = blk(x)
                out = [x.clone()]
                for blk in self.blocks[-self.last_skip:]:
                    x = blk(x)
        ...
        feat_patch = out[self.head_idx_patch][:, 1:]
        feat_cls = out[self.head_idx_cls][:, :1]
        feat_patch = self.norm(feat_patch)
        feat_cls = self.norm(feat_cls)

        bs, res2, d = feat_patch.shape
        h = w = int(math.sqrt(res2))
        feat_patch = feat_patch.reshape(bs,h,w,d).permute(0,3,1,2)
        return feat_cls.squeeze(1), feat_patch
    ...
    def get_middle_feature(self, x):
        ...
        h = w = int(math.sqrt(res2))
        feat_patch = feat_patch.reshape(bs,h,w,d).permute(0,3,1,2)
        return feat_cls.squeeze(1), feat_patch, None, None
