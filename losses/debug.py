...
class InnerSampleLoss(nn.Module):
    ...
    def forward_patch(self, outputs_gc_stu, outputs_gc_tea, outputs_lc_stu, \
        num_subsets, bbox_dict, masks=None, prefix='', head_idx=0, **kwargs):
        ...
        applied_subset = self.cfg.applied_subset_patch
        if applied_subset >= 0:
            gc_feat_stu = gc_feat_stu.chunk(num_subsets)[applied_subset]
            gc_feat_tea_sk = gc_feat_tea_sk.chunk(num_subsets)[applied_subset]
            lc_feat_stu = lc_feat_stu.chunk(num_subsets)[applied_subset]
            bboxes_gc = bboxes_gc.chunk(num_subsets)[applied_subset]
            bboxes_all = bboxes_all.chunk(num_subsets)[applied_subset]
        ...
        if masks is not None:
            _keep = 1 - masks.float().reshape(n_gc, bs, 1, h_gc, w_gc)
        else:
        ...
        for i, crop_id in enumerate(self.cfg.crops_for_assign):
            ...
            for v in np.delete(np.arange(np.sum(nmb_crops)), crop_id):
                ...
                aligned_log_p = torch.log_softmax(aligned_out / self.cfg.temperature, dim=1)
    ...
    def forward_cls(self, outputs_gc_stu, outputs_gc_tea, outputs_lc_stu, \
        num_subsets, prefix='', head_idx=0, **kwargs):
        ...
        applied_subset = self.cfg.applied_subset_cls
        if applied_subset >= 0:
            gc_feat_stu = gc_feat_stu.chunk(num_subsets)[applied_subset]
            gc_feat_tea_sk = gc_feat_tea_sk.chunk(num_subsets)[applied_subset]
            lc_feat_stu = lc_feat_stu.chunk(num_subsets)[applied_subset]
        ...
        for i, crop_id in enumerate(self.cfg.crops_for_assign):
            ...
            for v in np.delete(np.arange(np.sum(nmb_crops)), crop_id):
                if v in self.cfg.crops_for_assign:
                    ...
                    lc_index = v - nmb_crops[0]
                    out = lc_feat_stu[lc_index]
                log_p = torch.log_softmax(out / self.cfg.temperature, dim=1)
                # Mask cross entropy if attn mask was passed
    ...
    def forward(self, outputs_stu, outputs_tea, masks, lc_outputs_stu, \
        bboxes, projection_heads, num_subsets, pl_module, **kwargs):
        ...
        args = {
            ...
            'projection_head': projection_heads[0],
            'eps': 1e-5
        }
